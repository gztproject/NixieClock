...
void NixieClock::showIpFlashy(uint8_t *ip, uint8_t numOctets, uint16_t time)
{
    constrain(numOctets, 1, 4);
    constrain(time, 100, 10000);
    for (int i = 4 - numOctets; i < 4; i++)
    {
        //Show the octet for 3/4 time
        //showInt(ip[i],false); //That's boooriiing
        switch (i)
        {
        case 0:
            pinValues[0] = ip[i] > 9 ? itob(floor(ip[i] / 10), false) : EMPTY_PAIR;
            pinValues[1] = (itob(ip[i] % 10, false) >> 4) + EMPTY_RIGHT;
            pinValues[2] = EMPTY_PAIR;
            break;
        case 1:
            pinValues[0] = ip[i] > 99 ? itob(floor(ip[i] / 100), false) : EMPTY_PAIR;
            pinValues[1] = itob(ip[i] % 100, false);
            pinValues[2] = EMPTY_PAIR;
            break;
        case 2:
            pinValues[0] = EMPTY_PAIR;
            pinValues[1] = ip[i] > 9 ? itob(floor(ip[i] / 10), false) : EMPTY_PAIR;
            pinValues[2] = (itob(ip[i] % 10, false) >> 4) + EMPTY_RIGHT;
            break;
        case 3:
            pinValues[0] = EMPTY_PAIR;
            pinValues[1] = ip[i] > 99 ? itob(floor(ip[i] / 100), false) : EMPTY_PAIR;
            pinValues[2] = itob(ip[i] % 100, false);
            break;
        }
        ...
        //Blank out for 1/4 time
        pinValues[0] = EMPTY_PAIR;
        pinValues[1] = EMPTY_PAIR;
        pinValues[2] = EMPTY_PAIR;
        pinValues[3] = NO_DOTS;
        updateClock();
        delay(0.25 * time);
